...
ACPI_PATH0="/sys/class/power_supply/BAT0"
ACPI_PATH1="/sys/class/power_supply/BAT1"


# get battery status
STAT0=$(cat $ACPI_PATH0/status)
STAT1=$(cat $ACPI_PATH1/status)

# get remaining energy value
REM0=`grep "POWER_SUPPLY_ENERGY_NOW" $ACPI_PATH0/uevent | cut -d= -f2`
REM1=`grep "POWER_SUPPLY_ENERGY_NOW" $ACPI_PATH1/uevent | cut -d= -f2`
REM=$((REM0 + REM1))
# get full energy value
FULL0=`grep "POWER_SUPPLY_ENERGY_FULL_DESIGN" $ACPI_PATH0/uevent | cut -d= -f2`
FULL1=`grep "POWER_SUPPLY_ENERGY_FULL_DESIGN" $ACPI_PATH1/uevent | cut -d= -f2`
FULL=$((FULL0 + FULL1))
...
if [ $PERCENT -le "$(echo $LIMIT)" ] && ( [ "$STAT0" == $Discharging ] || [ "$STAT1" == $Discharging ] ); then
